...
export function setupGUI(parentContext) {
  // Hide selected GUI controls (user request).
  const HIDE_RESET_RELOAD_ACTUATORS = true;
  const HIDE_GUI_PANEL = true;
  ...
  parentContext.guiPauseController = pauseSimulation;
  // Store reference to paused text element
  let pausedTextElement = null;
  pauseSimulation.onChange((value) => {
    if (value) {
      // Remove existing paused text if it exists
      if (pausedTextElement && pausedTextElement.parentNode) {
        try {
          pausedTextElement.parentNode.removeChild(pausedTextElement);
        } catch (e) {
          console.warn('[GUI] Failed to remove existing paused text:', e);
        }
      }
      ...
      pausedText.style.zIndex = '1000';
      pausedText.innerHTML = 'pause';
      pausedTextElement = pausedText;
      if (parentContext.container && parentContext.container.appendChild) {
        parentContext.container.appendChild(pausedText);
      }
    } else {
      // Remove paused text element if it exists
      if (pausedTextElement && pausedTextElement.parentNode) {
        try {
          pausedTextElement.parentNode.removeChild(pausedTextElement);
          pausedTextElement = null;
        } catch (e) {
          console.warn('[GUI] Failed to remove paused text:', e);
          pausedTextElement = null;
        }
      }
    }
  ...
  document.addEventListener('keydown', (event) => {
    // Space is used for gripper toggle in teleop; keep pause shortcut to Shift+Space to avoid conflicts.
  ...
  keyInnerHTML += 'Shift+Space<br>';

  actionInnerHTML += 'Replay last trajectory<br>';
  keyInnerHTML += 'P<br>';

  if (!HIDE_RESET_RELOAD_ACTUATORS) {
  ...
  // Open by default only if the GUI panel is visible.
  if (!HIDE_GUI_PANEL) {
    parentContext.gui.open();
  }

  // Hide the whole GUI DOM if requested.
  if (HIDE_GUI_PANEL && parentContext.gui && parentContext.gui.domElement) {
    parentContext.gui.domElement.style.display = 'none';
    // Ensure it doesn't intercept pointer events even if display toggles later.
    parentContext.gui.domElement.style.pointerEvents = 'none';
  }
}
...
export async function loadSceneFromURL(mujoco, filename, parent) {
    ...
    try {
      console.log(`[MuJoCo] Loading model from /working/${filename}`);
    parent.model = mujoco.MjModel.loadFromXML("/working/"+filename);
      if (!parent.model) {
        throw new Error("MjModel.loadFromXML returned null - XML parsing failed");
      }
      console.log(`[MuJoCo] Model loaded successfully. nbody=${parent.model.nbody}, nq=${parent.model.nq}, nv=${parent.model.nv}`);
    } catch (error) {
      console.error(`[MuJoCo] Failed to load model from ${filename}:`, error);
      // Try to get more details about the error
      if (error.message) {
        console.error(`[MuJoCo] Error message: ${error.message}`);
      }
      // Check if the file exists in the virtual file system
      try {
        const fileInfo = mujoco.FS.analyzePath("/working/"+filename);
        console.log(`[MuJoCo] File exists in VFS: ${fileInfo.exists}`);
        if (fileInfo.exists) {
          const content = mujoco.FS.readFile("/working/"+filename, { encoding: 'utf8' });
          console.log(`[MuJoCo] File size: ${content.length} bytes`);
          console.log(`[MuJoCo] First 500 chars: ${content.substring(0, 500)}`);
        }
      } catch (fsError) {
        console.error(`[MuJoCo] Could not check file in VFS:`, fsError);
      }
      throw error;
    }
    try {
      console.log(`[MuJoCo] Creating MjData for model...`);
    parent.data  = new mujoco.MjData(parent.model);
      console.log(`[MuJoCo] MjData created successfully.`);
    } catch (error) {
      console.error(`[MuJoCo] Failed to create data for model from ${filename}:`, error);
      console.error(`[MuJoCo] Model stats: nbody=${parent.model.nbody}, nq=${parent.model.nq}, nv=${parent.model.nv}, nu=${parent.model.nu}`);
      // Try to get more details about the error
      if (error.message) {
        console.error(`[MuJoCo] Error message: ${error.message}`);
      }
    ...
    if (parent && typeof parent.onModelReloaded === 'function') {
      parent.onModelReloaded();
    }
...
/** Converts a THREE.Quaternion into MuJoCo's quaternion ordering/handedness
 * @param {THREE.Quaternion} quat */
export function toMujocoQuat(quat) {
  return {
    w: -quat.w,
    x: -quat.x,
    y: quat.z,
